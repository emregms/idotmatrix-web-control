...
export default function DeviceScanner({ onConnect, isConnected }: DeviceScannerProps) {
    ...
    const scan = async () => {
        ...
        try {
            ...
            setError("Tarama başarısız. Backend çalışıyor mu?");
            console.error(err);
        } finally {
            setScanning(false);
        }
    ...
    const connect = async (address: string) => {
        ...
        try {
            ...
            if (res.success) {
                onConnect();
                setDevices([]); // Clear list on success
            } else {
                setError("Bağlantı başarısız oldu.");
            }
        } catch (err) {
        ...
        return (
            <div className="p-4 bg-green-900/20 border border-green-500/50 rounded-lg flex items-center justify-between">
                <div className="flex items-center gap-3">
                    ...
                    <span className="text-green-400 font-medium">Cihaza Bağlı</span>
                </div>
                <button
                    onClick={async () => {
                ...
                {devices.map((device) => (
                    ...
                    >
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <div className="font-medium">{device.name || "Bilinmeyen Cihaz"}</div>
                        ...
                        >
                            {connecting === device.address ? "Bağlanıyor..." : "Bağlan"}
                        </button>
                    </div>
                ))}
                {devices.length === 0 && !scanning && !error && (
                    <div className="text-center text-slate-500 py-8">
                        Cihaz bulunamadı. Lütfen "Tara" butonuna basın.
                    </div>
                )}
            </div>
        </div>
